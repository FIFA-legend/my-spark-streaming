...
object StreamingJoins {
  ...
  def joinStreamWithStatic() = {
    ...
    // joins happens PER BATCH
    val streamedBandsGuitaristsDF = streamedBandsDF.join(guitarPlayers, guitarPlayers.col("band") === streamedBandsDF.col("id"), "inner")

    /*
      restricted joins:
      - streaming + static: RIGHT outer join/full outer join/right_semi not permitted
      - static + streaming: LEFT outer join/full/left_semi not permitted
     */
